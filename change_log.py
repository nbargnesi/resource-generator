...
# use dict annotation_name_to_file to remap anno dataset names to belanno
# file names
anno_match = True
            ...
            if 'ns' in d.scheme_type:
                if d.ids:
                    ...
                    id_prefix = id_prefix.replace(
                        'IDID', 'ID')  # catch EGID exception
                    id_prefix = id_prefix.replace(
                        'AFFXID', 'AFFX')  # catch AFFX exception
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--base_url",
        metavar="BASEURL",
        default="http://resource.belframework.org/belframework/",
    ...
    for prefix in old_ns_dict.keys():
        ...
        if prefix not in new_ns_dict.keys():
            print(
                "WARNING - Namespace {0} missing from new data".format(prefix))
            continue
        else:
            ...
            for term_id, label in old_ns_dict[prefix].items():
                ...
                # handle cases where ID changes and/or is withdrawn
                    ...
                    if label in new_ns_dict[prefix].values():
                        ...
                        print(
                            'WARNING! Label {0} appears valid but is associated with withdrawn/unresolved ID {1}:{2}'.format(
                                label, label_prefix, term_id))
                    if history_dict.get(
                            prefix) and term_id in history_dict[prefix]:
                        ...
                        if new_id_value is None:
                            ...
                            pass
                        # confirm new_id_value is valid
                        elif new_id_value not in new_ns_dict[prefix].keys():
                            new_id_value = 'unresolved'
                        else:  # if valid new_id_value from history
                            new_label_value = new_ns_dict.get(
                                prefix).get(new_id_value)
