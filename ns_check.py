...
parser = argparse.ArgumentParser(
    description='check resource-generator output files')
parser.add_argument(
    '-p',
    required=False,
    help='path for .belns and .beleq file directory')
...
def test_namespace_values(ns):
    ...
    if len(eq_dict.keys()) > 0 and len(ns_dict.keys()) > 0:
        ...
        if len(eq_dict.keys()) != len(ns_dict.keys()):
            print(ns + ' .beleq {0} and .belns {1} value number mismatch!'.format(
                len(eq_dict.keys()), len(ns_dict.keys())))
            check = False
            extra_vals = set(ns_dict.keys()).symmetric_difference(
                set(eq_dict.keys()))
...
for n in namespaces:
    ...
    print(
        '{1} uuids for {0} values in {2}.beleq'.format(
            ns_length,
            ns_unique,
            n))

# report number of equivalenced values for gene and protein namespaces to
# root (entrez Gene)
for n in gp_namespaces:
    (matches, ns1_length) = compare_namespace_equivalences(n, gp_root)
    print(
        '{0} of {1} values in {2} are equivalenced to {3}'.format(
            matches,
            ns1_length,
            n,
            gp_root))
...
(matches, ns1_length) = compare_namespace_equivalences(
    'swissprot-ids', 'swissprot')
print('{0} of {1} values in {2} are equivalenced to {3}'.format(
    matches, ns1_length, 'swissprot-accession-numbers', 'swissprot-entry-names'))
