...
// under the License.
package org.openbel.reggie.rdf.generate.equivalences;
...
/**
 * Generates BEL equivalences from the RDF.
 */
public class Main {
    ...
    void generate(QuerySolutions nsConceptIter, EquivalenceTemplate template) {
        ...
        for (QuerySolution nsConcept : nsConceptIter) {
            ...
            EquivalenceConcept concept = new EquivalenceConcept(triples);
    ...
    public static void main(String... args) throws Exception {
        ...
        String env = getenv("RG_EQ_OUTPUT");
        if (env == null) {
            err.println("RG_EQ_OUTPUT is not set");
            exit(1);
        }
        File f = new File(env);
        if (!f.exists()) {
            ...
            if (!rslt && !f.exists()) {
                err.println("error creating equivalence output path " + env);
                exit(1);
            }
        }
